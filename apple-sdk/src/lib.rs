...
impl Platform {
    ...
    /// Attempt to construct an instance from a target triplet.
    ///
    /// The argument should be a target triplet of rust toolchain. e.g.
    /// `x86_64-apple-darwin`.
    ///
    /// Will return [Error::UnknownTarget] if this does not appear to be a known
    /// target triplet.
    pub fn from_target_triplet(target: &str) -> Result<Self, Error> {
        let platform = match target {
            ...
            "aarch64-apple-ios-macabi" => Platform::IPhoneOs,
            ...
            "i386-apple-tvos" | "x86_64-apple-tvos" => Self::AppleTvSimulator,
            target if target.ends_with("-apple-tvos-sim") => Self::AppleTvSimulator,
...
mod test {
    ...
    fn target_platform() -> Result<(), Error> {
        ...
        fn test(target: &str, platform: Platform) {
            assert_eq!(Platform::from_target_triplet(target).unwrap(), platform);
        }
        ...
        // test("x86_64-apple-ios-macabi", IPhoneOs);  // simulator?
        test("aarch64-apple-tvos", AppleTvOs);
        test("x86_64-apple-tvos", AppleTvSimulator);

        assert!(Platform::from_target_triplet("x86_64-unknown-linux-gnu").is_err());
